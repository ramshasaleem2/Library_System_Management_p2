...
ADD CONSTRAINT FK_member FOREIGN KEY (issued_member_id) REFERENCES members (member_id);

ALTER TABLE issued_status
ADD CONSTRAINT FK_book FOREIGN KEY (issued_book_isbn) REFERENCES books (isbn);

ALTER TABLE issued_status
ADD CONSTRAINT FK_employee FOREIGN KEY (issued_emp_id) REFERENCES employees (emp_id);

ALTER TABLE employees
ADD CONSTRAINT FK_branch FOREIGN KEY (branch_id) REFERENCES branch (branch_id);

ALTER TABLE return_status
ADD CONSTRAINT FK_book_isbn Foreign Key (return_book_isbn) REFERENCES books (isbn);
...
SELECT a.*, b.manager_id, a1.emp_name as manager
FROM
    employees as a
    JOIN branch as b on a.branch_id = b.branch_id
    JOIN employees as a1 ON b.manager_id = a1.emp_id;

-- Task 11. Create a Table of Books with Rental Price Above a Certain Threshold 7USD:
CREATE TABLE books_price_morethan7
sELECT *
from books
WHERE
    rental_price > 7;
...
SELECT ist.issued_member_id, a.member_name, b.book_title, ist.issued_date, rs.return_date, CURRENT_DATE - ist.issued_date as days_overdue
from
    issued_status as ist
    JOIN members as a ON a.member_id = ist.issued_member_id
...
-- Use the CREATE TABLE AS (CTAS) statement to create a new table active_members containing members who have issued at least one book in the last 32 months.
...
where
    member_id IN (
        ...
        from issued_status
        where
            issued_date >= CURRENT_DATE - INTERVAL 32 MONTH
    )
